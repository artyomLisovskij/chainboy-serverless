...
def get_docker_client(
    ...
    if client_key not in _DOCKER_CLIENTS:
        client = docker.DockerClient(
            base_url=base_url or config.DOCKER_URL, timeout=config.DOCKER_TIMEOUT
        )
        # retries = Retry(
...
def inspect_container_node(container):
    ...
    try:
        ...
        logger.exception("Failed to inspect the container", container=container)
...
def docker_communicate(container, stdin=None, start_container=True, timeout=None):
    """
    ...
    sock = docker_client.api.attach_socket(container, params=params)
    sock._sock.setblocking(False)  # Make socket non-blocking
    log.info(
        "Attached to the container", params=params, fd=sock.fileno(), timeout=timeout
    )
    if not stdin:
