...
def _create_sandbox_container(
    ...
    try:
        c = docker_client.api.create_container(
            image,
            command=command,
            user=user,
            stdin_open=True,
...
def start(sandbox, stdin=None):
    ...
    try:
        stdout, stderr = utils.docker_communicate(
            sandbox, stdin=stdin, timeout=realtime_limit
        )
    except TimeoutError:
...
def destroy(sandbox):
    ...
    docker_client = utils.get_docker_client()
    try:
        docker_client.api.remove_container(sandbox, v=True, force=True)
...
class _WorkingDirectory(object):
    ...
    def __repr__(self):
        return "WorkingDirectory(volume={!r}, node={!r})".format(self.volume, self.node)
...
def _write_files(container, files):
    ...
    try:
        docker_client.api.put_archive(
            container, config.DOCKER_WORKDIR, tarball_fileobj.getvalue()
        )
    except (RequestException, DockerException) as e:
        log.exception(
